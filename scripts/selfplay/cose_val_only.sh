...
# =============================================================================
# COSE: Base-model benchmark evaluation (val_only mode)
#
# Runs ONE benchmark evaluation over the 19-benchmark suite on the chosen
# base model and exits. No training, no rollouts beyond benchmark eval.
# Numbers are directly comparable to the in-training `val/benchmark_accuracy`
# metric (same pipeline, same gpt-4.1-nano judge).
#
# Usage:
#     sbatch scripts/selfplay/cose_val_only.sh                              # uses default MODEL
#     MODEL=Qwen/Qwen3-8B sbatch scripts/selfplay/cose_val_only.sh
#     MODEL=meta-llama/Llama-3.1-8B-Instruct sbatch scripts/selfplay/cose_val_only.sh
#     MODEL=Qwen/Qwen2.5-3B-Instruct sbatch scripts/selfplay/cose_val_only.sh
#
# The MODEL environment variable picks the base (or trained) checkpoint.
# Hydra overrides `trainer.val_only=true` and `trainer.val_before_train=true`
# at the bottom of the launch line trigger eval-then-exit behavior.
# =============================================================================
...
# NOTE: vLLM's CuMemAllocator asserts against expandable_segments:True — do NOT set
# PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True here or engine init will crash.
mkdir -p "${TMPDIR}"

# Benchmark-evaluation judge (same gpt-4.1-nano as the in-training eval).
...
# Stable run name with the model identifier so WandB groups eval-only runs
# together for cross-model comparison.
MODEL_TAG=$(echo "${MODEL}" | tr '/' '-' | tr ':' '-')
RUN_NAME=COSE_EVAL_${MODEL_TAG}_$(date +%Y%m%d_%H%M%S)
